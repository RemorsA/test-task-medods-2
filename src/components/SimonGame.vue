<template>
	<div class="simonGameWrap">
		<div class="simonCircleWrap" :class="{ 'beingHighlighted': isHighlighting, 'gameNotOn': !isGameOn }">
			<div v-for="sector in sectors" :key="sector.id" :class="{ [sector.class]: true, 'sector_highlighted': sector.isHighlighted }" @click="handleSectorClick(sector.id)"></div>
		</div>
		<div class="infoAndControls">
			<div class="setLevelButtons">
				<RadioInput flex_row :name="languageButtons.name" :buttons="languageButtons.buttons" :defaultValue="languageButtons.value" @update-input-data="setLanguage"/>
			</div>
			<p v-if="!isUserFail" class="roundNumber"> {{languagesLocales.round}}: {{roundNumber}}</p>
			<div @click="startTheGame">
				<Button v-if="!isGameOn" type="button" filled :label="languagesLocales.startTheGameButtonLabel"/>
			</div>
			<p v-if="isUserFail" class="lossAlert"> {{languagesLocales.youLostAlert}}</p>
			<div @click="repeatHighlight">
				<Button v-if="isGameOn" :disabled="isHighlighting" type="button" hollow :label="languagesLocales.repeatHighlightButtonLabel"/>
			</div>
			<div class="setLevelButtons">
				<RadioInput flex_column :disabled="isHighlighting" :name="levelButtons.name" :buttons="levelButtons.buttons" :defaultValue="levelButtons.value" @update-input-data="setLevel"/>
			</div>
			<div @click="tryLostRound">
				<Button v-if="isUserFail" type="button" hollow :label="languagesLocales.tryLostRoundButtonLabel"/>
			</div>
		</div>
	</div>
</template>
